...
class DataIngestionConfig:
    ...
    raw_data_dir: str= f"{DATASET_DIR}{os.sep}raw"
    shared_dir: str= f"{DATASET_DIR}{os.sep}raw{os.sep}shared"
    preprocess_dir: str= f"{DATASET_DIR}{os.sep}raw{os.sep}preprocess"
    final_image_dir: str = f"{DATASET_DIR}{os.sep}raw{os.sep}final_images"
    split_image_dir: str = f"{DATASET_DIR}{os.sep}raw{os.sep}split_images"
...
class DataIngestion:
    ...
    def initiate_data_ingestion(self,
        ...
        try:
            ...
            if not os.path.exists(self.config.final_image_dir):
                logging.info("Creating final images directory")
                self.copy_images_to_dir(preprocess_dir=self.config.preprocess_dir, final_image_dir=self.config.final_image_dir)

            if not os.path.exists(f'{self.config.raw_data_dir}{os.sep}match.xlsx'):
                logging.info("Preprocessing xlsx file and Combining match and nonmatches")
            self.copy_master_inventory_to_dir(shared_dir=self.config.shared_dir,raw_data_dir=self.config.raw_data_dir)
            ...
            #     logging.info("Splitting Images")
            # self.split_images(split_image_dir_name=split_image_dir_name)
            # self.correct_rotated_images(raw_data_dir=self.config.raw_data_dir, tape_dir=f'{self.config.split_image_dir}{os.sep}{split_image_dir_name}')
    ...
    def preprocess_xlsx(self):
        df_nonmatch = pd.read_excel(f"{self.config.raw_data_dir}{os.sep}nonmatch.xlsx").drop(columns = 'Unnamed: 0')
        df_match = pd.read_excel(f"{self.config.raw_data_dir}{os.sep}match.xlsx").drop(columns = 'Unnamed: 0')
        ...
        df_nonmatch.to_excel(f"{self.config.raw_data_dir}{os.sep}nonmatch.xlsx")
        df_match.to_excel(f"{self.config.raw_data_dir}{os.sep}match.xlsx")
    ...
    def combine_match_nonmatch(self, df_match, df_nonmatch):
        ...
        df.to_excel(f"{self.config.raw_data_dir}{os.sep}match-nonmatch.xlsx")
        return None
